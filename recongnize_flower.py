...
class PlantRecognizer():
    ...
    def __init__(self):
        #init key
        self.key_list = [{'api_key':'G5Vq7N0GGTBGK5C3vk4BV2N7', 'secret_key':'2NqvsF'},
              {'api_key':'0BQDyq70rwrqvYG4Pxqlq6cI', 'secret_key':'thNFUnkeIN'},
              {'api_key':'wgxFAFYzf1DNxtfpx0ltGOWV', 'secret_key':'00SjqCtYbTPU'},
              {'api_key':'wRGCrs8LMKLns6o3PncVmybg', 'secret_key':'HyGWBUO'},
              {'api_key':'nR4crblf18okFgqEFMHKEUaZ', 'secret_key':'rE'},
              {'api_key':'bcGMFf1wvOq6OuvcxvMBuaoG', 'secret_key':'t'},
              {'api_key':'1X5jrxzIC9wuXBx3iBFUwmv9', 'secret_key':''},
              {'api_key':'WzR0OroAKIE7xwiKjPGRTpcL', 'secret_key':'Hq'},
              {'api_key':'hNOvEkxktGbctqGbtbyAQE1g', 'secret_key':'x'},
              {'api_key':'l5H7LmEOQ6ngxQj6V91i9sFI', 'secret_key':'9'},#
              {'api_key':'byeXlLkdGWvCAoPPPK6gsyRe', 'secret_key':'A9'},
              {'api_key':'IxyvfymjcacoKk5TNqyKjLPO', 'secret_key':''}]
        self.jj = 0  #order read key
        self.API_URL = None
    ...
    def start(self, path):
        ...
        for single_class in contents:
            ...
            for ii,image in enumerate(os.listdir(single_class_path)):
                ...
                self.access_token = self._get_access_token(self.key_list[self.jj]['api_key'],self.key_list[self.jj]['secret_key'])
                self.detect(img_path) 
                # update jj
                self.jj = (self.jj+1) % 10
